...
#define DATA_MAX        16384u

#define PD_COMID_SUB        1000u
#define PD_COMID_CYCLE  500000u             /* in us (1000000 = 1 sec) */
#define OWN_IP_ADDR "192.168.1.28"
...
// cJson
void merge_json_objects(cJSON *dst, cJSON *src)
{
    cJSON *src_element = NULL;
    cJSON_ArrayForEach(src_element, src)
    {
        cJSON *dst_element = cJSON_GetObjectItem(dst, src_element->string);
        if (!dst_element)
        {
            cJSON_AddItemToObject(dst, src_element->string, cJSON_Duplicate(src_element, 1));
        }
    }
}
...
int main ()
{
    ...
    // cJson counter and all dev data
    int json_dev_conunter = 0;
    cJSON *json_merged = NULL;
    while (1)
    {
        TRDP_FDS_T          rfds;
        INT32               noDesc;
        TRDP_TIME_T         tv;
        const TRDP_TIME_T   max_tv  = {0, PD_COMID_CYCLE * 3};
        const TRDP_TIME_T   min_tv  = {0, PD_COMID_CYCLE};
        ...
        if ((TRDP_NO_ERR == err)
            && (receivedSize > 0))
        {
            if (json_dev_conunter == 0)
            {
                json_merged = cJSON_Parse(gBuffer);
                json_dev_conunter += 1;
            }
            else if (json_dev_conunter < 2)
            {
                cJSON *json_current = cJSON_Parse(gBuffer);

                cJSON *cu_pub_key_merged = cJSON_GetObjectItem(json_merged, "cu_pub_key");
                cJSON *cu_pub_key_current = cJSON_GetObjectItem(json_current, "cu_pub_key");
                cJSON *dev_pub_key_dict_merged = cJSON_GetObjectItem(json_merged, "dev_pub_key_dict");
                cJSON *dev_pub_key_dict_current = cJSON_GetObjectItem(json_current, "dev_pub_key_dict");
                cJSON *data_merged = cJSON_GetObjectItem(json_merged, "data");
                cJSON *data_current = cJSON_GetObjectItem(json_current, "data");

                merge_json_objects(dev_pub_key_dict_merged, dev_pub_key_dict_current);
                merge_json_objects(data_merged, data_current);
                if (cu_pub_key_current != NULL && cu_pub_key_merged == NULL)
                {
                    cu_pub_key_current = cJSON_Duplicate(cu_pub_key_current, 1);
                    cJSON_AddItemToObject(json_merged, "cu_pub_key", cu_pub_key_current);
                }
                
                cJSON_Delete(json_current);
                json_dev_conunter += 1;
            }
            if (json_dev_conunter == 2)
            {
                char *merged_json_str = cJSON_Print(json_merged);
                // socket client
                // Send message to server
                if (merged_json_str != NULL)
                {
                    sendto(sockfd2, merged_json_str, strlen(merged_json_str), 0, (struct sockaddr *)&client_addr, sizeof(client_addr));
                    printf("recv trdp to socket: %s\n", merged_json_str);
                }
                //---socket client
                free(merged_json_str);
                cJSON_Delete(json_merged);
                json_merged = NULL;
                json_dev_conunter = 0;
            }
